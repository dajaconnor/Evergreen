...
SelfCheckManager.prototype.drawHolds = function(holds) {
	...
	if(data.status == 4) {
		...
		if(dojo.date.stamp.fromISOString(data.hold.capture_time())<(new Date())) this.byName(row2, 'date').style.color="red";
		this.byName(row2, 'date').innerHTML = dojo.date.locale.format(dojo.date.stamp.fromISOString(data.hold.capture_time()), {selector: 'date', fullYear: true});
...
SelfCheckManager.prototype.handleXactResult = function(action, item, result) {
    ...
    this.handleAlert(displayText, sound);
    return {};
}
