...
function osrfMessage(hash) {
	...
	// This will alert the staff client user of DB INSERT errors
	if (hash.hasOwnProperty("type")
	&& hash.type == "STATUS"
	&& hash.hasOwnProperty("payload")
	&& hash.payload.hasOwnProperty("__p")
	&& hash.payload.__p.hasOwnProperty("statusCode")
	&& hash.payload.__p.statusCode == OSRF_STATUS_INTERNALSERVERERROR
	&& hash.payload.__p.hasOwnProperty("status")){
	
		alert(hash.payload.__p.status);
	}
